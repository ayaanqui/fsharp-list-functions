...
let rec length L =
    let rec _length L len = 
        match L with
        | [] -> len
        | _::tail -> _length tail (len + 1)
    _length L 0
...
let max L =
    let greater n1 n2 = if (n1 > n2) then n1 else n2

    let rec _max L hi = 
        match L with
        | [] -> hi
        | elem::tail -> _max tail (greater hi elem)

    _max (List.tail L) (List.head L)
...
let rec fold F start L =
    match L with
    | [] -> start
    | elem::tail -> fold F (F start elem) tail
...
let unzip L =
    let rec _unzip L tuple =
        match L, tuple with
        | [], (a, b) -> (a::[], b::[])
        | e::tail, (a, b) ->
            let (x, y) = _unzip tail e
            (a::x, b::y)
    
    if L = [] then ([], [])
    else _unzip (List.tail L) (List.head L)
...
let range stop =
    let rec _range L stop = 
        if stop = 0 then L
        else _range ((stop-1)::L) (stop - 1)
...
let range2 start stop =
    let rec _range2 L start stop = 
        if stop = start then L
        else _range2 ((stop-1)::L) start (stop - 1)
...
let slice L start stop =
    let rec _slice L start stop i =
        if i >= stop then
            []
        else if i >= start then
            List.head L::_slice (List.tail L) start stop (i+1)
        else
            _slice (List.tail L) start stop (i+1)
...
let rec filter F L =
    ...
    | [] -> []
    | elem::tail -> 
        if F elem then elem :: (filter F tail)
        else filter F tail
...
let pairUp L =
    let rec _pairUp L findingPair prev =
        match L with
        | [] -> []
        | elem::tail ->
            if findingPair then 
                (prev, elem) :: (_pairUp tail false elem)
            else _pairUp tail true elem

    if L = [] then []
    else _pairUp (List.tail L) true (List.head L)
...
let inline distanceFromOrigin (x,y) = sqrt (float(x*x) + float(y*y))
...
let lengthsFromOrigin L =
    let rec _lengthsFromOrigin L point =
        match L, point with
        | [], (x, y) -> [distanceFromOrigin(x, y)]
        | elem::tail, (x, y) -> distanceFromOrigin(x, y) :: _lengthsFromOrigin tail elem

    if L = [] then []
    else _lengthsFromOrigin (List.tail L) (List.head L)
